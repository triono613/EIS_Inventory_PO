...
interface MemberContractLoaderProps {
    dataState: State,
    memberId: string,
    searchText: string,
    version: number,
...
export const MemberContractLoader = (props: MemberContractLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props.memberId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/contract/gridPerMember?${queryString}`
        const params = {
