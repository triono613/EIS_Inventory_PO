...
interface AvlUnitListLoaderProps {
    dataState: State,
    searchText: string,
    avlunitlistId: string | null,
    version: number,
    onDataReceived: (products: DataResult) => void
}

export const AvlUnitListLoader = (props: AvlUnitListLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props.avlunitlistId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/avlunitlist/grid?${queryString}`
        const params = {
