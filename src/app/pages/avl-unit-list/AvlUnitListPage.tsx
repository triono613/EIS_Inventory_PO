...
import { AvlUnitListLoader } from './AvlUnitListLoader'
import AvlUnitListForm from './AvlUnitListForm'
...
export function AvlUnitListPage() {
    const { t } = useTranslation('translation')
    const dispatch = useDispatch()
    const [gridHeight, setGridHeight] = useState(0)
    const [avlunitlistId, setAvlUnitListId] = useState<string | null>(null)
    ...
    function onFilterAvlUnitListIdChange(value: string | null) {
        setAvlUnitListId(value)
    }
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.deviceId })
        } else {
            message = t('DataHasBeenUpdated', { data: data.deviceId })
        }
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('AvlUnitList'),
            dataId: data.device_id,
        })
    }


    function handleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('AvlUnitList'), data.deviceId, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.deviceId })
        let successMsg = t('DataHasBeenDeleted', { data: data.deviceId })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/avlunitlist/delete/${data.device_id}`
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('AvlUnitList')}</PageTitle>
            ...
            <ContentContainer>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            <GridColumn field='device_id' title={t('AvlUnitListDevice')} width='200px' />
                            <GridColumn field='serial_number' title={t('AvlUnitListSerial')} width='200px' />
                            <GridColumn field='imei' title={t('AvlUnitListImei')} width='260px' />
                            <GridColumn field='avl_unit_id' title={t('AvlUnitListId')} width='260px' />
                            <GridColumn field='avl_unit_type_desc' title={t('AvlUnitListDesc')} width='260px' />
                            <GridColumn field='purchase_date' title={t('AvlUnitListDate')} format='{0:dd-MMM-yyyy}' width='260px' />
