...
interface AccessoriesLoaderProps {
    dataState: State,
    searchText: string,
...
export const AccessoriesLoader = (props: AccessoriesLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props. accessoriesId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/accessories/grid?${queryString}`
        const params = {
