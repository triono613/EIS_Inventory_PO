...
interface PrivilegesLoaderProps {
    dataState: State,
    searchText: string,
...
export const PrivilegesLoader = (props: PrivilegesLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props.privilegeId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/privilege/grid?${queryString}`
        const params = {
