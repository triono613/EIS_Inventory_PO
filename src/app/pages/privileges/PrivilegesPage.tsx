...
import { PrivilegesLoader } from './PrivilegesLoader'
...
export function PrivilegesPage() {
    ...
    const [privilegeId, setPrivilegeId] = useState<string | null>(null)
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.privilegeName })
        } else {
            message = t('DataHasBeenUpdated', { data: data.privilegeName })
        }
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('Privilege'),
            dataId: data.privilege_id,
        })
    }

    function handleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('Privilege'), data.privilegeName, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.privilegeName })
        let successMsg = t('DataHasBeenDeleted', { data: data.privilegeName })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/Privilege/delete/${data.privilege_id}`
    ...
    const privilegeNameCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.privilege_name}</a></td>
        )
    }



    const privilegeCodeCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.privilege_code}</a></td>
        )
    }
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('Privilege')}</PageTitle>
            ...
            <ContentContainer>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            <GridColumn field='privilege_name' title={t('PrivilegeName')} cell={privilegeNameCell} width='200px' />
                            <GridColumn field='privilege_code' title={t('PrivilegeCode')} cell={privilegeCodeCell} width='260px' />
