...
export const loadGridData = createAsyncThunk<DataSourceResult, LoadParameter>(
    'supplier/loadGridData',
    async (param: LoadParameter) => {
        const url = `api/supplier/grid?${param.queryString}`
        const params = {
...
export const loadData = createAsyncThunk<Supplier, string>(
    'supplier/loadData',
    async (param: string) => {
        const url = `api/supplier/${param}`
...
export const { gridStateChanged, gridPageChanged, userGridLoaded, dataLoading, dataLoaded, newData } = supplierSlice.actions
