...
interface GsmCardListLoaderProps {
    dataState: State,
    searchText: string,
    gsmcardlistId: string | null,
    version: number,
    onDataReceived: (products: DataResult) => void
}

export const GsmCardListLoader = (props: GsmCardListLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props.gsmcardlistId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/gsmcardlist/grid?${queryString}`
        const params = {
