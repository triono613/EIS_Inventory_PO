...
import { GsmCardListLoader } from './GsmCardListLoader'
import GsmCardListForm from './GsmCardListForm'
...
export function GsmCardListPage() {
    const { t } = useTranslation('translation')
    const dispatch = useDispatch()
    const [gridHeight, setGridHeight] = useState(0)
    const [gsmcardlistId, setGsmCardListId] = useState<string | null>(null)
    ...
    function onFilterGsmCardListIdChange(value: string | null) {
        setGsmCardListId(value)
    }
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('GsmCardList'),
            dataId: data.gsm_card_id,
        })
    }


    function handleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('GsmCardList'), data.Id, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.Id })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/gsmcardlist/delete/${data.gsm_card_id}`
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('GsmCardList')}</PageTitle>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            ...
                            <GridColumn field='gsm_number' title={t('GsmListNumber')} width='200px' />
                            <GridColumn field='locked' title={t('Locked')} cell={CheckMarkCell}  width='200px' />
                            <GridColumn field='purchase_date' title={t('GsmCardListDate')} format='{0:dd-MMM-yyyy}' width='260px' />
                            <GridColumn field='owned' title={t('Owned')} cell={CheckMarkCell} width='200px' />
                            <GridColumn field='status' title={t('Status')} width='200px' />
                            <GridColumn field='member_name' title={t('CustomerName')} width='200px' />
                            <GridColumn field='vehicle_number' title={t('VehicleNumber')} width='200px' />
                            <GridColumn field='notes' title={t('Notes')} width='200px' />
