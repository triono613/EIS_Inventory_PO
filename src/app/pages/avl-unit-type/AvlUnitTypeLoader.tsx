...
interface AvlUnitTypeLoaderProps {
    dataState: State,
    searchText: string,
    avlunittypeId: string | null,
    version: number,
    onDataReceived: (products: DataResult) => void
}

export const AvlUnitTypeLoader = (props: AvlUnitTypeLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props. avlunittypeId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/avlunittype/grid?${queryString}`
        const params = {
