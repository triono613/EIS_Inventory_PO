...
import { AvlUnitTypeLoader } from './AvlUnitTypeLoader'
import AvlUnitTypeForm from './AvlUnitTypeForm'
...
export function AvlUnitTypePage() {
    const {t} = useTranslation('translation')
    const dispatch = useDispatch()
    const [gridHeight, setGridHeight] = useState(0)
    const [avlunittypeId, setAvlUnitTypeId] = useState<string|null>(null)
    ...
    function onFilterAvlUnitTypeIdChange(value:string|null) {
        setAvlUnitTypeId(value)
    }
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.avlUnitTypeCode })
        } else {
            message = t('DataHasBeenUpdated', { data: data.avlUnitTypeCode })
        }
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('AvlUnitType'),
            dataId: data.avl_unit_type_id,
        })
    }


    function handleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('AvlUnitType'), data.avlUnitTypeCode, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.avlUnitTypeCode })
        let successMsg = t('DataHasBeenDeleted', { data: data.avlUnitTypeCode })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/avlunittype/delete/${data.avl_unit_type_id}`
    ...
    const avlUnitTypeCodeCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.avl_unit_type_code}</a></td>
        )
    }

    const avlUnitTypeDescCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.avl_unit_type_desc}</a></td>
        )
    }
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('AvlUnitType')}</PageTitle>
            ...
            <ContentContainer>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            <GridColumn field='avl_unit_tye_code' title={t('AvlUnitTypeCode')} cell={avlUnitTypeCodeCell} width='200px' />
                            <GridColumn field='avl_unit_tye_desc' title={t('AvlUnitTypeDesc')} cell={avlUnitTypeDescCell} width='260px' />
