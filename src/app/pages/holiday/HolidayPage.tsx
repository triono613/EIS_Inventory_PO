...
export function HolidayPage() {
    ...
    function onFilterHolidayIdChange(value:string|null) {
        setHolidayId(value)
    }
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.date })
        } else {
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('Holiday'),
            dataId: data.id,
        })
    }

    function hadleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('Holiday'), data.description, () => {
            hadleDeleteConfirm(data)
        })
    }

    function hadleDeleteConfirm(data: any) {
        dispatch(closeDialog(null))
        let loadingMsg = t('DeletingData', { data: data.description })
        let successMsg = t('DataHasBeenDeleted', { data: data.description })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/holiday/delete/${data.id}`
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('Holiday')}</PageTitle>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            ...
                            <GridColumn field='date' title={t('HolidayDate')} format='{0:dd-MMM-yyyy}' width='200px' />
                            <GridColumn field='description' title={t('Description')}  width='260px' />
