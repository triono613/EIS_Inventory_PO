...
import { GsmProviderLoader } from './GsmProviderLoader'
import GsmProviderForm from './GsmProviderForm'
...
export function GsmProviderPage() {
    const {t} = useTranslation('translation')
    const dispatch = useDispatch()
    const [gridHeight, setGridHeight] = useState(0)
    const [gsmproviderId, setGsmProviderId] = useState<string|null>(null)
    ...
    function onFilterGsmProviderIdChange(value:string|null) {
        setGsmProviderId(value)
    }
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.gsmProviderName })
        } else {
            message = t('DataHasBeenUpdated', { data: data.gsmProviderName })
        }
    ...
    function handleEditButtonClick(data: any) {
        setEditState({
            visible: true,
            mode: 'edit',
            title: t('Edit') + ' ' + t('GsmProvider'),
            dataId: data.gsm_provider_id,
        })
    }

    function handleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('GsmProvider'), data.gsmProviderName, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.gsmProviderName })
        let successMsg = t('DataHasBeenDeleted', { data: data.gsmProviderName })
    ...
    function submitDeletePromise(data: any) {
        let url = `api/gsmprovider/delete/${data.gsm_provider_id}`
    ...
    const gsmProviderNameCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.gsm_provider_name}</a></td>
        )
    }

    const gsmPrefixesCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.gsm_prefixes}</a></td>
        )
    }

    const inventoryItemCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => handleEditButtonClick(props.dataItem)}  >{props.dataItem.inventory_item_id}</a></td>
        )
    }
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('GsmProvider')}</PageTitle>
            ...
            <ContentContainer>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            <GridColumn field='gsm_provider_name' title={t('GsmProviderName')} cell={gsmProviderNameCell} width='200px' />
                            <GridColumn field='gsm_prefixes' title={t('GsmPrefixes')} cell={gsmPrefixesCell} width='260px' />
                            <GridColumn field='inventory_item_id' title={t('GsmProviderInventory')} cell={inventoryItemCell} width='260px' />
