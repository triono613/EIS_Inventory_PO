...
interface GsmProviderLoaderProps {
    dataState: State,
    searchText: string,
...
export const GsmProviderLoader = (props: GsmProviderLoaderProps) => {
    ...
    const requestDataIfNeeded = () => {
        ...
        let key = queryString + '|' + props. gsmproviderId + '|' + props.searchText + '|' + props.version;
        if (pending.current || key === lastSuccess.current) {
            console.log('exit')
            return
        }
        pending.current = key;
        const url = `api/gsmprovider/grid?${queryString}`
        const params = {
