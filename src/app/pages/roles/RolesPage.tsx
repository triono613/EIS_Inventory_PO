...
export function RolesPage() {
    ...
    function handleFormSuccess(data: any) {
        ...
        if (editState.mode === 'add') {
            message = t('DataHasBeenAdded', { data: data.name })
        } else {
            message = t('DataHasBeenUpdated', { data: data.name})
        }
    ...
    function hadleDeleteButtonClick(data: any) {
        ShowDeleteConfirmation(dispatch, t, t('Role'), data.name, () => {
    ...
    function handleDeleteConfirm(data: any) {
        ...
        let loadingMsg = t('DeletingData', { data: data.name})
        let successMsg = t('DataHasBeenDeleted', { data: data.name})
    ...
    function submitDeletePromise(data: any) {
        let url = `api/Role/delete/${data.id}`
    ...
    const nameCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => hadleEditButtonClick(props.dataItem)}  >{props.dataItem.name}</a></td>
        )
    }

    const normalizedNameCell = (props: GridCellProps) => {
        return (
            <td width='200px'><a href='#' onClick={() => hadleEditButtonClick(props.dataItem)}  >{props.dataItem.normalizedName}</a></td>
        )
    }
    ...
    return (
        <PageContainer>
            <PageTitle breadcrumbs={[]}>{t('Role')}</PageTitle>
            ...
            <ContentContainer>
                ...
                <div id='gridContainer' className='w-100 h-100'>
                    {gridHeight > 0 ? (
                        ...
                        >
                            <GridColumn field='name' title={t('Name')} cell={nameCell} width='200px' />
                            <GridColumn field='normalizedName' title={t('NormalizedName')} cell={normalizedNameCell} width='260px' />
